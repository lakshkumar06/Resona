...
const handleStopAndDone = () => {
  ...
  setRecording(false); // Stop the recording
  setDoneClicked(false); // Ensure the done button is not yet clicked
};
  ...
  const createWatermarkOnServer = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        console.log("Watermark created:", data);
      } else {
    ...
    <div className='px-[10vw] pt-[10vh]  pb-[5vh] h-screen flex flex-col justify-between text-white'>
      <div>
        ...
        {recording && nextCount < 11 && (
          <p className="text-[22px] font-medium text-[#B7B7B7] text-center mt-[1em]">"Technology has transformed the way we communicate, learn, and interact with the world..."</p>
        )}
      </div>
...
>
  Stop
        ...
        {!recording && nextCount < 10 && (
          <div className="">
            <button onClick={handleStartNextClick} className="mx-auto aspect-square px-[20px] bg-white rounded-[100px] relative bottom-0 flexCol">
              Start Next
            </button>
            <p className="text-center text-[20px] font-semibold mt-[0.5em]">Start Next</p>
          </div>
        )}
